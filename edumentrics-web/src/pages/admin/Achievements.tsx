...
export default function Achievements() {
  const { certificates, updateCertificate, achievements, updateAchievement, students, currentUser } = useAppContext();
  ...
  // Normalize certificates and achievements into one list
  const normalizedItems = [
    ...certificates.map(c => ({
      ...c,
      itemType: 'certificate',
      normalizedStatus: c.status,
      studentName: students.find(s => s.id === c.studentId)?.name || 'Noma\'lum talaba'
    })),
    ...achievements.map(a => ({
      ...a,
      itemType: 'achievement',
      normalizedStatus: a.status === 'kutilmoqda' ? 'Pending' : a.status === 'tasdiqlangan' ? 'Approved' : 'Rejected',
      studentName: students.find(s => s.id === a.studentId)?.name || 'Noma\'lum talaba'
    }))
  ];

  const filteredItems = normalizedItems.filter(item => {
    const matchesTab = item.normalizedStatus === activeTab;
  ...
  const stats = {
    Pending: normalizedItems.filter(i => i.normalizedStatus === 'Pending').length,
    Approved: normalizedItems.filter(i => i.normalizedStatus === 'Approved').length,
    Rejected: normalizedItems.filter(i => i.normalizedStatus === 'Rejected').length
  };

  const handleApprove = () => {
    if (!selectedItem) return;

    if (selectedItem.itemType === 'certificate') {
      updateCertificate(selectedItem.id, { 
        status: 'Approved',
        pointsAwarded: approveForm.score
      });
    } else {
      updateAchievement(selectedItem.id, 'tasdiqlangan', approveForm.score);
    }

    toast.success(`✓ Tasdiqlandi! +${approveForm.score} ball ${selectedItem.studentName}ga qo'shildi`, {
    ...
    setApproveForm({ type: 'faollik', score: 4, comment: '' });
  };
  ...
  const handleReject = () => {
    ...
    if (selectedItem.itemType === 'certificate') {
      ...
      updateAchievement(selectedItem.id, 'rad_etilgan');
    }

    toast.success("Yutuq rad etildi. Talabaga xabar yuborildi.", {
      position: 'bottom-right',
    });
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-[#E8EFED]">
        <div>
          <h1 className="text-[22px] font-bold text-[#0D1F1A] tracking-[-0.3px]">Yutuqlarni ko'rib chiqish</h1>
      ...
      {filteredItems.length > 0 ? (
          ...
          {filteredItems.map((item) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="font-bold text-[#0D1F1A] truncate max-w-[200px]">{item.studentName}</h4>
                  <p className="text-[11px] text-[#9DB4AB] font-medium uppercase mt-0.5 tracking-tight">{item.type || item.category}</p>
                </div>
                <span className={cn(
                  "px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider",
                  item.normalizedStatus === 'Pending' ? "bg-[#FFFBEB] text-[#D97706]" :
                  item.normalizedStatus === 'Approved' ? "bg-[#EDF7F4] text-[#0A9B82]" :
                  "bg-[#FEF2F2] text-[#DC2626]"
                )}>
                  {item.normalizedStatus === 'Pending' ? 'KUTILMOQDA' : 
                   item.normalizedStatus === 'Approved' ? 'TASDIQLANGAN' : 'RAD ETILGAN'}
                </span>
              </div>
              ...
              <div className="space-y-2 mb-6 flex-1">
                <div className="flex items-center gap-2 text-[#6B8A80]">
                  ...
                  <span className="text-xs font-semibold">{new Date(item.uploadDate || item.date).toLocaleDateString()}</span>
                </div>
                {(item.organization || item.result) && (
                  <div className="flex items-center gap-2 text-[#6B8A80]">
                    <Building className="w-3.5 h-3.5 text-[#9DB4AB]" />
                    <span className="text-xs font-semibold truncate">{item.organization || item.result}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-[#6B8A80]">
                  <FileText className="w-3.5 h-3.5 text-[#9DB4AB]" />
                  <span className="text-xs font-semibold">{item.itemType === 'certificate' ? 'Sertifikat fayli' : 'Yutuq tasdig\'i'}</span>
                </div>
              </div>

              <div className="flex items-center gap-2 mt-auto">
                <button 
                  onClick={() => { setSelectedItem(item); setShowPreviewModal(true); }}
              ...
              {item.normalizedStatus === 'Approved' && (
                <div className="mt-4 pt-4 border-t border-[#F0F4F3] flex items-center gap-2 text-[#0A9B82]">
                  ...
                  <span className="text-xs font-black uppercase tracking-tight">Berilgan ball: +{item.pointsAwarded || item.points || 4}</span>
                </div>
              )}
              ...
              {item.normalizedStatus === 'Rejected' && (
                <div className="mt-4 pt-4 border-t border-[#F0F4F3]">
                  <p className="text-[11px] text-[#DC2626] font-medium leading-relaxed">
                    <span className="font-bold">Rad etilish sababi:</span> {item.rejectionReason || 'Izohsiz rad etilgan'}
                  </p>
                </div>
              )}
            </motion.div>
        ...
        <div className="py-20 bg-white rounded-2xl border border-[#E8EFED] flex flex-col items-center justify-center text-center px-6">
          ...
          <p className="text-[13px] text-[#6B8A80] mt-1 max-w-[280px]">Ushbu bo'limda {activeTab === 'Pending' ? 'kutilayotgan' : activeTab === 'Approved' ? 'tasdiqlangan' : 'rad etilgan'} yutuqlar mavjud emas.</p>
        </div>
      )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-[#0D1F1A]">Yutuqni tasdiqlash</h3>
              ...
              <div className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                    {['Sertifikat', 'Musobaqa', 'Mentorlik', 'Volontyorlik'].map(t => (
                      <button 
                        key={t}
                        onClick={() => setApproveForm(prev => ({ ...prev, type: t, score: t === 'Sertifikat' ? 5 : t === 'Musobaqa' ? 10 : 3 }))}
                        className={cn(
      ...
      <AnimatePresence>
        {showRejectModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-[#0D1F1A]">Yutuqni rad etish</h3>
              ...
              <div className="space-y-4">
                <p className="text-sm text-[#6B8A80] font-medium leading-relaxed">
                  Yutuqni rad etayotganingizning sababini yozing. Bu xabar talabaga yuboriladi.
                </p>
                <div className="space-y-1.5">
            ...
            >
              <div className="p-6 border-b border-[#E8EFED] flex items-center justify-between bg-[#F8FFFE]">
                <div>
                  ...
                  <p className="text-[12px] text-[#9DB4AB] font-bold uppercase tracking-tight mt-0.5">Yutuq ID: {selectedItem?.id?.slice(0, 8)}</p>
                </div>
              ...
              <div className="p-8 bg-[#F3F4F6] min-h-[400px] flex items-center justify-center">
                <div className="w-full max-w-md aspect-[1.4/1] bg-white rounded-lg shadow-inner border-2 border-dashed border-[#D1D5DB] flex flex-col items-center justify-center text-center p-8">
                  ...
                  <h4 className="text-lg font-bold text-[#0D1F1A]">{selectedItem?.studentName} - Yutuq fayli</h4>
                  <p className="text-[13px] text-[#6B8A80] mt-1">Ushbu joyda yutuqning PDF yoki rasm fayli ko'rsatiladi.</p>
                  <div className="mt-8 flex gap-4">
                    <div className="text-left px-4 py-3 bg-white border border-[#E8EFED] rounded-xl">
                      ...
                      <p className="text-sm font-bold text-[#0D1F1A] tracking-tight">{selectedItem?.studentId}</p>
                    </div>
                    <div className="text-left px-4 py-3 bg-white border border-[#E8EFED] rounded-xl">
                      <p className="text-[10px] font-bold text-[#9DB4AB] uppercase tracking-wider mb-0.5">Turi</p>
                      <p className="text-sm font-bold text-[#0D1F1A] tracking-tight">{selectedItem?.type || selectedItem?.category}</p>
                    </div>
                  </div>
                </div>
              </div>

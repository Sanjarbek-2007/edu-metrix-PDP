...
import { 
  ...
  TrendingDown, AlertTriangle, CheckCircle2, Hash, Home
} from 'lucide-react';
...
export default function StudentProfile() {
  ...
  const { 
    ...
    certificates, violations, updateCertificate, messages, addMessage, achievements, updateAchievement 
  } = useAppContext();
  
  const student = students.find(s => s.id === id);
  const [activeTab, setActiveTab] = useState<'Score' | 'Attendance' | 'Certificates' | 'Violations' | 'Messages' | 'Room'>('Score');
    ...
    <div className="space-y-6 pb-14">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <p className={cn(
                ...
                student.scores.finalScore >= 85 ? "text-[#0A9B82]" : student.scores.finalScore >= 75 ? "text-[#D97706]" : "text-[#DC2626]"
              )}>{student.scores.finalScore}</p>
              <span className="text-xl font-bold text-[#9DB4AB]">/100+</span>
            </div>
            <p className="text-[12px] text-[#6B8A80] font-medium mt-1 italic">Asosiy ball: {student.scores.asosiyTotal}</p>
          </div>
          <div className="w-16 h-16 rounded-full border-4 border-[#F3F4F6] flex items-center justify-center relative">
            <svg className="w-16 h-16 absolute inset-0 -rotate-90">
              <circle 
                ...
                strokeDasharray={`${(Math.min(100, Math.max(0, student.scores.finalScore)) / 100) * 176} 176`}
              />
            </svg>
            <Award className={cn(
              "w-6 h-6",
      ...
      <div className="bg-white rounded-[14px] border border-[#E8EFED] shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden">
        <div className="flex overflow-x-auto no-scrollbar border-b border-[#E8EFED] bg-[#F8FFFE]">
          ...
          <TabButton id="Room" label="Yotoqxona" icon={Home} />
          ...
          <AnimatePresence mode="wait">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[11px] font-bold text-[#9DB4AB] uppercase tracking-wider">{categoryNames[change.category] || change.category}</span>
                          <span className="w-1 h-1 rounded-full bg-[#E8EFED]" />
            ...
            {activeTab === 'Certificates' && (
              ...
              >
                {allYutuqlar.map((item) => (
                  <div key={item.id} className="p-4 bg-white border border-[#E8EFED] rounded-xl flex items-center justify-between group hover:border-[#0A9B82]/30 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-[#0D1F1A] text-[14px] truncate max-w-[200px]">{item.title}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className={cn(
                            "px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider",
                            item.status === 'Approved' ? "bg-[#EDF7F4] text-[#0A9B82]" : 
                            item.status === 'Pending' ? "bg-[#FFFBEB] text-[#D97706]" : "bg-[#FEF2F2] text-[#DC2626]"
                          )}>
                            {item.status === 'Approved' ? 'TASDIQLANGAN' : 
                             item.status === 'Pending' ? 'KUTILMOQDA' : 'RAD ETILGAN'}
                          </span>
                          <span className="w-1 h-1 rounded-full bg-[#E8EFED]" />
                          <span className="text-[11px] font-medium text-[#6B8A80]">{item.date}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      {item.status === 'Approved' && (
                        <span className="text-[11px] font-black text-[#0A9B82]">+{item.points || 5} ball</span>
                      )}
                      {item.status === 'Pending' && (
                        <div className="flex gap-2">
                           <button 
                             ...
                             className="px-3 py-1.5 bg-[#0A9B82] text-white rounded-lg text-[9px] font-black uppercase tracking-wider hover:bg-[#087D6A] transition-all"
                           >
                             Tasdiqlash
                           </button>
                           <button 
                             onClick={() => {
                               if (item.itemType === 'certificate') {
                                 updateCertificate(item.id, { status: 'Rejected', rejectionReason: 'Mezonlarga tushmadi' });
                               } else {
                                 updateAchievement(item.id, 'rad_etilgan');
                               }
                               toast.error('Rad etildi');
                             }}
                             className="px-3 py-1.5 bg-[#FEF2F2] text-[#DC2626] rounded-lg text-[9px] font-black uppercase tracking-wider hover:bg-[#FEE2E2] transition-all border border-[#FECACA]"
                           >
                             Rad etish
                           </button>
                        </div>
            ...
            {activeTab === 'Violations' && (
              <motion.div 
                ...
                className="space-y-6"
              >
                <div className="flex justify-between items-center bg-[#FEF2F2] p-4 rounded-2xl border border-[#FECACA]">
                  <div>
                    <h4 className="text-sm font-bold text-[#991B1B]">Intizom nazorati</h4>
                    <p className="text-[11px] text-[#DC2626] font-medium">Jami jarimalar: {studentViolations.reduce((acc, v) => acc + (v.finePoints || v.pointsDeducted), 0)} ball</p>
                  </div>
                  <button className="px-4 py-2 bg-white border border-[#FECACA] text-[#DC2626] rounded-xl text-xs font-bold hover:bg-[#DC2626] hover:text-white transition-all shadow-sm">
                    Yangi qoidabuzarlik qo'shish
                  </button>
                </div>
            ...
            {activeTab === 'Room' && (
              <motion.div 
                key="room" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                className="space-y-8"
              >
                <div className="flex flex-col items-center justify-center py-6">
                  <div className="w-full max-w-md bg-[#F8FFFE] border border-[#E8EFED] rounded-2xl p-8 text-center">
                    <div className="w-16 h-16 bg-[#EDF7F4] rounded-2xl flex items-center justify-center text-[#0A9B82] mx-auto mb-4">
                      <Home className="w-8 h-8" />
                    </div>
                    <h4 className="text-lg font-bold text-[#0D1F1A]">Xona: {student.roomNumber || 'Yotoqxonada turmaydi'}</h4>
                    <p className="text-[13px] text-[#6B8A80] mt-2 font-medium">Yotoqxona intizomi va tozalik ko'rsatkichlari</p>
                    
                    {student.roomNumber && (
                      <div className="grid grid-cols-2 gap-4 mt-8">
                         <div className="p-4 bg-white border border-[#E8EFED] rounded-xl">
                            <p className="text-[11px] font-bold text-[#9DB4AB] uppercase tracking-wider mb-2">Tozalik</p>
                            <p className="text-xl font-black text-[#0A9B82]">A+</p>
                         </div>
                         <div className="p-4 bg-white border border-[#E8EFED] rounded-xl">
                            <p className="text-[11px] font-bold text-[#9DB4AB] uppercase tracking-wider mb-2">Navbatchilik</p>
                            <p className="text-xl font-black text-[#0D1F1A]">100%</p>
                         </div>
                      </div>
                    )}
                  </div>
                ...
                {student.roomNumber && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <h4 className="text-sm font-bold text-[#0D1F1A] uppercase tracking-wider">So'nggi kirish-chiqish</h4>
                      <div className="bg-white border border-[#E8EFED] rounded-xl overflow-hidden">
                        <table className="w-full text-left text-xs">
                          <thead className="bg-[#F8FFFE] text-[#9DB4AB] uppercase font-bold">
                            <tr>
                          ...
                          <tbody className="divide-y divide-[#E8EFED]">
                            <tr>
                              <td className="px-4 py-3 font-medium">Bugun, 08:12</td>
                              <td className="px-4 py-3"><span className="text-[#DC2626] font-bold">CHIQISH</span></td>
                            </tr>
                            <tr>
                              <td className="px-4 py-3 font-medium">Kecha, 21:45</td>
                              <td className="px-4 py-3"><span className="text-[#0A9B82] font-bold">KIRISH</span></td>
                            </tr>
                            <tr>
                              <td className="px-4 py-3 font-medium">Kecha, 14:20</td>
                              <td className="px-4 py-3"><span className="text-[#DC2626] font-bold">CHIQISH</span></td>
                            </tr>
                          </tbody>
                        </table>
                    ...
                    <div className="space-y-4">
                      <h4 className="text-sm font-bold text-[#0D1F1A] uppercase tracking-wider">Ruxsat so'rovlari</h4>
                      <div className="space-y-3">
                        {useAppContext().absenceRequests.filter(r => r.studentId === student.id).slice(0, 3).map(req => (
                          <div key={req.id} className="p-3 bg-white border border-[#E8EFED] rounded-xl flex items-center justify-between">
                            <div>
                              <p className="text-[13px] font-bold text-[#374151]">{req.reason}</p>
                              <p className="text-[11px] text-[#6B8A80]">{req.startDate} - {req.endDate}</p>
                            </div>
                            <span className={cn(
                              "px-2 py-0.5 rounded text-[10px] font-black uppercase",
                              req.status === 'Approved' ? "bg-emerald-50 text-[#0A9B82]" : 
                              req.status === 'Pending' ? "bg-amber-50 text-[#D97706]" : "bg-rose-50 text-[#DC2626]"
                            )}>
                              {req.status === 'Approved' ? 'TASDIQLANGAN' : req.status === 'Pending' ? 'KUTILMOQDA' : 'RAD ETILGAN'}
                            </span>
                          </div>
                        ))}
                        {useAppContext().absenceRequests.filter(r => r.studentId === student.id).length === 0 && (
                          <div className="p-8 text-center text-[#9DB4AB] bg-slate-50 border border-dashed border-[#E8EFED] rounded-xl">
                            <p className="text-xs font-medium">Surovlar mavjud emas</p>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      { id: 'amaliy', name: 'Amaliy ko\'nikma' },
                      ...
                      <button 
                        key={cat.id}
                        onClick={() => setPointForm(prev => ({ ...prev, category: cat.id }))}
                        className={cn(
                          "py-2.5 rounded-lg border text-[11px] font-bold transition-all truncate px-1",
                          pointForm.category === cat.id ? "bg-[#EDF7F4] border-[#0A9B82] text-[#0A9B82]" : "border-[#D1D5DB] text-[#6B8A80] hover:bg-slate-50"
                        )}
                      >
                        {cat.name}
                      </button>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     ...
                     <span className="text-2xl font-black text-[#0A9B82]">{pointForm.amount}</span>
                  </div>
                  ...
                  <div className="relative pt-4 pb-2">
                    <input 
                       ...
                       onChange={e => setPointForm(prev => ({ ...prev, amount: parseInt(e.target.value) || 1 }))}
                       className="w-full h-2 bg-[#E8EFED] rounded-lg appearance-none cursor-pointer accent-[#0A9B82]"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="flex-1 py-3 rounded-xl bg-[#FEF2F2] border border-[#FECACA] flex items-center justify-center gap-2 text-[#DC2626] font-black text-sm hover:bg-[#FEE2E2] transition-all active:scale-95"
                    >
                    ...
                    <div className="w-20 relative group shrink-0">
                      <input 
                        ...
                        value={pointForm.amount}
                        onChange={e => setPointForm(prev => ({ ...prev, amount: Math.min(50, Math.max(1, parseInt(e.target.value) || 1)) }))}
                        className="w-full h-12 px-2 bg-white border-2 border-[#E8EFED] rounded-xl text-lg font-black text-[#0D1F1A] focus:border-[#0A9B82] outline-none shadow-sm transition-all text-center"
                      />
                    </div>
                    <button 
                      onClick={() => setPointForm(prev => ({ ...prev, amount: Math.min(50, prev.amount + 1) }))}
                      className="flex-1 py-3 rounded-xl bg-[#EDF7F4] border border-[#0A9B82]/20 flex items-center justify-center gap-2 text-[#0A9B82] font-black text-sm hover:bg-[#E2E8E5] transition-all active:scale-95"
                    >
                      ...
                      <button 
                        ...
                        className={cn(
                          ...
                          pointForm.amount === val 
                            ...
                            : "bg-[#F4F7F6] border-[#E8EFED] text-[#6B8A80] hover:border-[#0A9B82]/40 hover:text-[#0A9B82]"
                        )}
                      >
                        {val}
                      </button>

...
function ScoringConfigTab() {
  const fixedWeights = [
    { key: 'Akademik natija', val: 40 },
    { key: 'Davomat', val: 20 },
    { key: 'Amaliy ko\'nikmalar', val: 15 },
    { key: 'Faollik va Sertifikatlar', val: 10 },
    { key: 'Tyutor bahosi', val: 5 },
    { key: 'Intizom (Korporativ)', val: 10 },
  ];
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white border border-[#E8EFED] rounded-[16px] p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#F0F4F3]">
            <div className="p-2 bg-[#F8FFFE] rounded-lg">
              <Sliders className="w-5 h-5 text-[#0A9B82]" />
            </div>
            <h3 className="font-bold text-[#0D1F1A] text-lg">Ballar vazni (100 ball shkalasi)</h3>
          </div>
          <p className="text-[12px] font-medium text-[#6B8A80] mb-6 italic bg-[#F9FBFA] p-3 rounded-lg border border-[#E8EFED]">Bu ballar PDP University rasmiy nizomi asosida belgilangan va ularni o'zgartirib bo'lmaydi.</p>
          
          <div className="space-y-4">
            {fixedWeights.map((w) => (
              <div key={w.key} className="flex justify-between items-center py-2 border-b border-dashed border-[#E8EFED] last:border-0">
                <span className="text-[13px] font-bold text-[#374151]">{w.key}</span>
                <span className="text-[13px] font-black text-[#0A9B82] bg-[#EDF7F4] px-2 py-0.5 rounded">{w.val} ball</span>
              </div>
            ))}
            <div className="pt-4 flex justify-between items-center mt-2">
              <span className="text-[14px] font-black uppercase text-[#9DB4AB] tracking-widest">Jami Asosiy ball:</span>
              <span className="text-xl font-black text-[#0A9B82]">100 ball</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="bg-white border border-[#E8EFED] rounded-[16px] p-8 shadow-sm">
            <h3 className="text-lg font-bold text-[#0D1F1A] mb-6">Grant chegaralari (Tahrirlash mumkin)</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-[#F9FBFA] border border-[#E8EFED] rounded-xl focus-within:border-[#0A9B82] transition-colors">
                <div>
                  <span className="text-[14px] font-bold text-[#0D1F1A] block">Unicorn minimal ball</span>
                </div>
                <input type="number" defaultValue={80} className="w-16 bg-white border border-[#E8EFED] rounded-lg py-2 text-center font-bold text-[#0A9B82] shadow-sm outline-none focus:border-[#0A9B82]" />
              </div>
              <div className="flex items-center justify-between p-4 bg-[#F9FBFA] border border-[#E8EFED] rounded-xl focus-within:border-[#0A9B82] transition-colors">
                <div>
                  <span className="text-[14px] font-bold text-[#0D1F1A] block">Golden Mind minimal ball</span>
                </div>
                <input type="number" defaultValue={85} className="w-16 bg-white border border-[#E8EFED] rounded-lg py-2 text-center font-bold text-[#0A9B82] shadow-sm outline-none focus:border-[#0A9B82]" />
              </div>
              <div className="flex items-center justify-between p-4 bg-[#F9FBFA] border border-[#E8EFED] rounded-xl focus-within:border-red-500 transition-colors">
                <div>
                  <span className="text-[14px] font-bold text-[#0D1F1A] block">GPA minimal daraja (%)</span>
                  <p className="text-[10px] text-[#6B8A80] mt-0.5">80% dan past bo'lsa grant saqlanmaydi</p>
                </div>
                <input type="number" defaultValue={80} className="w-16 bg-white border border-[#E8EFED] rounded-lg py-2 text-center font-bold text-red-500 shadow-sm outline-none focus:border-red-500" />
              </div>
            </div>
          </div>
          
          <div className="bg-white border border-[#E8EFED] rounded-[16px] p-8 shadow-sm">
            <h3 className="text-lg font-bold text-[#0D1F1A] mb-6">Jarima va Reabilitatsiya</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-[#F9FBFA] border border-[#E8EFED] rounded-xl">
                <div>
                  <span className="text-[14px] font-bold text-[#0D1F1A] block">Bir semestr u. max jarima</span>
                </div>
                <input type="number" defaultValue={-20} className="w-20 bg-white border border-[#E8EFED] rounded-lg py-2 text-center font-bold text-red-500 shadow-sm outline-none" />
              </div>
              <div className="flex items-center justify-between p-4 bg-[#F9FBFA] border border-[#E8EFED] rounded-xl">
                <div>
                  <span className="text-[14px] font-bold text-[#0D1F1A] block">Reabilitatsiya max ball</span>
                </div>
                <input type="number" defaultValue={10} className="w-16 bg-white border border-[#E8EFED] rounded-lg py-2 text-center font-bold text-[#0A9B82] shadow-sm outline-none" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-[#E8EFED] rounded-[16px] p-8 shadow-sm">
         <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-[#0D1F1A]">Mentorlarni boshqarish</h3>
            <button className="px-4 py-2 bg-[#0A9B82] text-white text-xs font-bold rounded-lg hover:bg-[#087D6A]">Yangi mentor qo'shish</button>
         </div>
         <div className="overflow-x-auto border rounded-xl border-[#E8EFED]">
            <table className="w-full text-left text-sm">
               <thead className="bg-[#F8FFFE] border-b border-[#E8EFED] text-[11px] font-black uppercase text-[#9DB4AB] tracking-widest">
                  <tr>
                     <th className="p-4">Mentor Ismi</th>
                     <th className="p-4">Email</th>
                     <th className="p-4 text-center">Biriktirilgan Talabalar</th>
                     <th className="p-4 text-right">Amallar</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-[#E8EFED]">
                  <tr>
                     <td className="p-4 font-bold text-[#0D1F1A]">Aziz Karimov</td>
                     <td className="p-4 text-[#6B8A80]">mentor@pdpu.uz</td>
                     <td className="p-4 text-center"><span className="font-bold text-[#0A9B82] bg-[#EDF7F4] px-3 py-1 rounded-lg">3 ta talaba</span></td>
                     <td className="p-4 text-right"><button className="text-[#0A9B82] font-bold text-xs hover:underline">Tahrirlash</button></td>
                  </tr>
               </tbody>
            </table>
         </div>
      ...
      <div className="flex justify-end pt-4">
        <button onClick={() => toast.success('Sozlamalar saqlandi')} className="px-8 py-3 bg-[#0A9B82] hover:bg-[#087D6A] text-white font-bold rounded-xl shadow-md transition-all active:scale-95 flex items-center justify-center gap-2">
          <Save className="w-5 h-5" /> O'zgarishlarni saqlash
        </button>
      </div>
    </motion.div>
  );
